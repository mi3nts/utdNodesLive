function [] = liveRunTLLPeriod(nodeIndex,yamlFile,daysBack)
    display("---------------------MINTS---------------------")
    ...
    %% For different days  - The rest remains the same 
...
%     GPSGPRMC2Files   =  dir(strcat(todaysNodeFolder,'/*GPSGPRMC2*.csv'));
    ...
    predictedTable = [GPSGPGGA2,predictedTablePre,predictedTablePost];
    ...
    xlabel('time')
    ylabel('')
    legend('PM_{1}','PM_{2.5}','PM_{4}','PM_{10}')
    title("PM levels for Node " + nodeID)
