function [] = liveRunDynamicM(nodeIndex,yamlFile)
    ...
    lengthFile         =  strcat(scanFolder,"x"+nodeID,".mat")
    folderCheck(lengthFile)
    display(lengthFile);
...
%     GPSGPRMC2Files   =  dir(strcat(todaysNodeFolder,'/*GPSGPRMC2*.csv'));
    ...
    if csvAvailable
        currentLength =  load(lengthFile).nextLength; 
        In  =  table2array(latestDataAll(currentLength:end,mintsInputs));
        predictedTablePre = latestDataAll(currentLength:end,contains(latestDataAll.Properties.VariableNames,"binCount"));
        strCombine = "predictedTablePost = timetable(latestDataAll.dateTime(currentLength:end,:)";
    else
        currentLength =  1;
    ...
    nextLength = height(latestDataAll)-1;
...
%% Plotting - No Plotting for Dynamic Code  
